...
Used to compute costs for best INEv plan and centralized evaluation for new network configuration.
...
def main():
   ...
   # print("Hop Latency: " + str(hoplatency))
    ...
    schema = ["ID", "Swaps", "Skew", "INEvCosts", "CentralizeCosts"]
    myResult = [experiment, swaps, skew, outdated, costs, ccosts[0]]
    new = False
    
    try:
        f = open("adaptivity/results_inev.csv")   
    except FileNotFoundError:
        new = True           
        
    with open("adaptivity/results_inev.csv", "a") as result:
