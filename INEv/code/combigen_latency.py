...
with open('processingLatency', 'rb') as processingLatency_file: 
        processingLatency = pickle.load(processingLatency_file)  
        
centralLatency = processingLatency[1]        
hopfactor = processingLatency[2]
...
def extractMsOptions(query, myprojlist):
...
def MSoptionsPerEvent(query, myprojlist):
    events =  extractMsOptions(query, myprojlist) 
    MSOptionsDict = {}
    for event in events:
        MSOptionsDict[event] = []
        for proj in myprojlist:
...
def getBestChainCombis(query, shared, criticalMSTypes):     
    ...
    myMSDict = MSoptionsPerEvent(query, myprojlist)     
    
    
    for projection in [x for x in myprojlist]: #trivial combination and ms placement for all projections 
            ...
            if partType:                
                ...
                combiDict[projection] = (projection.leafs(), partType, costs, latencyEstimate(projection, projection.leafs(),hopfactor))
    ...
    for projection in sorted([x for x in myprojlist if len(x.leafs()) > 2], key = lambda x: len(x.leafs())):  # returns combination, that has only one input projection, the rest are primitive event types
                ...
                for mytuple in myMSDict[eventtype]:
                    if projection == mytuple[1]: 
                        ...
                        if res:     
                            ...
                            myLatency = latencyEstimate(projection, mycombination,hopfactor)
                            if myLatency < centralLatency:
                                if curcosts > mycosts: # update with new best chain combination
                                    mycosts = curcosts
                                    combiDict[projection] = (mycombination, [eventtype], mycosts, myLatency)
            ...
            mylist = [x for x in myprojlist if len(x.leafs()) < len(projection.leafs()) and set(x.leafs()).issubset(projection.leafs()) ]#and not combiDict[x][3] > centralLatency ]     
    ...
    if mylist:
       ...
       if not mycombi or set(sum([[x] if len(x) == 1 else x.leafs() for x in mycombi],[])) != set(projection.leafs()):  #not even one ms placeable subprojection exists ?
           return
                       ...
                       (mycosts, partEvent, myLatency) = costsOfCombination(projection, mycombi, shared, criticalMSTypes)  
                       
                       if not projection in combiDict.keys():             # projection has only sis placement and thus was not in combidict before
                           combiDict[projection] = (mycombi, partEvent, mycosts, myLatency)         
...
def costsOfCombination(projection, mycombi, shared, criticalMSTypes): # here is a good spot to check the combinations that are actually enumerated during our algorithm
       ...
       if partEvent:           
          ...
          mycosts -= (sum(list(map(lambda x: totalRate(x), [y for y in mycombi if not (y in combiDict.keys() and combiDict[y][1])])))) * longestPath 
       ...
       myLatency = latencyEstimate(projection, mycombi,hopfactor)
       
       if latencyEstimate(projection, mycombi,hopfactor) > centralLatency:
            
            mycosts = -np.inf
       
       return (mycosts, partEvent, myLatency) 
   

def latencyEstimate(projection, mycombi, hopfactor):  # estimate latency (processing latency  +  hop latency) for given combination of projection under given hopfactor 
    myMax = max([combiDict[x][3] for x in mycombi if len(x) > 1]+[0])
    myMax += hopfactor * maxDist # here it would be easy to model the difference between ms>sis and sis>ms steps
    #myMax += hopfactor * longestPath * 1.5
    mycombi = unfold_combi(projection, mycombi)
    processingLatency = getLatency_Diamond(projection, mycombi)
    myLatency = myMax + processingLatency

    return myLatency



def depth(projection, mycombi):
    mydepth = 1
    if set(mycombi) == set(projection.leafs()): # has no complex ancestors
        return mydepth   
    else:   
        mydepth += max(list(map(lambda x: depth(x, combiDict[x][0]), [x for x in mycombi if len(x)>1])))
        return mydepth 
...
def eventSharing(projection, mycombi, mycosts, shared): 
    ...
    #TODO: to be extended to multi-query scenario        
    return costs
